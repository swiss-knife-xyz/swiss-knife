...
import SessionRequestModal from "../wallet/bridge/components/SessionRequestModal";
import { SessionRequest } from "../wallet/bridge/types";
...
function AppStoreContent({
  ...
  const handleBack = () => {
    router.push(`${getPath(subdomains.APPS.base)}`);
    setAppUrl(null);
  };
  ...
  const openDapp = (url: string) => {
    ...
    setIsIframeLoading(true);
    router.push(
      `${getPath(subdomains.APPS.base)}?url=${encodeURIComponent(url)}`
    );
    setAppUrl(url);
  };
